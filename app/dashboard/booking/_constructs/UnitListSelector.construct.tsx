...
import { faChevronRight, faMobileAlt, faSearch, faTimes } from "@fortawesome/free-solid-svg-icons";
  ...
  return (
    <div className="w-full flex flex-col gap-2">
      ...
      <button
        ...
        className={cn(
          "w-full border rounded-lg px-4 py-3 flex items-center gap-3 transition-all text-left bg-white",
          invalid ? "border-danger" : "border-gray-200",
          selectedUnit && "border-primary ring-1 ring-primary/30 bg-primary/5"
        )}
      >
        {selectedUnit ? (
          <>
            <div
              ...
              style={{ backgroundColor: selectedUnit.unit_category?.color || "#7b9cff" }}
            >
            ...
            <FontAwesomeIcon icon={faChevronRight} className="text-xs text-on-surface-variant flex-shrink-0" />
          </>
        ) : (
          <>
            <div className="w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 bg-gray-100">
              <FontAwesomeIcon icon={faMobileAlt} className="text-lg text-gray-400" />
            </div>
            <div className="flex-grow min-w-0">
              <p className="text-sm text-on-surface-variant">Pilih unit...</p>
            </div>
            <FontAwesomeIcon icon={faChevronRight} className="text-xs text-on-surface-variant flex-shrink-0" />
          </>
        )}
      ...
      {/* Unit selection panel — custom overlay, no BottomSheet to avoid history/swipe conflict */}
      {typeof document !== "undefined" && createPortal(
        <>
          {/* Backdrop */}
          <div
            className={cn(
              "fixed inset-0 bg-black/40 z-[9999] transition-opacity duration-200",
          ...
          <div
            className={cn(
              "fixed bottom-0 left-0 right-0 z-[10000] bg-white rounded-t-2xl shadow-xl transition-transform duration-300 ease-out",
          ...
          >
            ...
            <div className="flex flex-col gap-3 p-4 pt-0">
              <div className="flex items-center justify-between">
                <h6 className="font-bold text-lg text-foreground">Pilih Unit</h6>
                <button
                  type="button"
                  onClick={() => setShowPanel(false)}
                  className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-gray-100 transition-colors"
                >
                  <FontAwesomeIcon icon={faTimes} className="text-sm text-on-surface-variant" />
                </button>
              </div>
              ...
              <div className="flex flex-col gap-2 overflow-y-auto input-scroll pr-1" style={{ maxHeight: "calc(75vh - 140px)" }}>
                {loading
