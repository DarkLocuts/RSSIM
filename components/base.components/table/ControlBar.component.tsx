...
export function ControlBarComponent({
  ...
  return (
    <>
      <div className={cn("py-1 md:py-2 md:px-1.5 bg-white md:border rounded-lg flex items-center mb-4", className)}>
        {(isSm ? [
          ...(options?.filter((op, iop) => iop == 0 || op == "SEARCH") || []), 
          ...(options && options.filter((op) => op != "SEARCH")?.length > 1 ? ['MOBILE_OPTION'] : [])
            ...
            return (
              <div className={cn("w-full min-w-[150px]", searchable ? "pr-1.5" : "px-1.5")} key={key}>
                <InputComponent
                  ...
                  className={cn("py-2 md:py-1.5 md:text-sm", searchable && "rounded-l-none")}
                />
              </div>
            );
          }
      ...
      {isSm && (
        ...
        >
          <div className='flex flex-col gap-4 p-2'>
            {options?.filter((op, iop) => (iop != 0 && op != "CREATE" && op != "REFRESH" && op != "SEARCH"))?.map((option: ControlBarOptionType, key: number) => {
              {
                // =========================>
                // ## Search Field
                // =========================>
